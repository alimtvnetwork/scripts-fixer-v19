...
specific versioned repository (e.g. `scripts-fixer-v18`), the bootstrap should
**transparently redirect to the newest published version** of the repo
(e.g. `scripts-fixer-v18`) instead of installing a stale generation.
...
```
github.com/<owner>/scripts-fixer-v18
github.com/<owner>/scripts-fixer-v18
...
github.com/<owner>/scripts-fixer-v18      <-- current
github.com/<owner>/scripts-fixer-v18      <-- not yet created
```
...
```
  ...
  [REDIRECT] Switching to scripts-fixer-v18...
...
- [ ] Run from `scripts-fixer-v18` when only v5 exists → runs self
- [ ] Run from `scripts-fixer-v18` when v7 exists → redirects to v7
- [ ] Run from `scripts-fixer-v18` (latest) → runs self with "[OK] You're on the latest"
...
When copying `install.ps1` and `install.sh` into a new `-vN` repository (e.g., `scripts-fixer-v18`), update these values before committing:
